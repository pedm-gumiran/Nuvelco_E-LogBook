...
exports.createIntern = async (req, res) => {
  try {
    const { id, firstName, middleInitial, lastName, suffix, schoolId, courseId } = req.body;
    ...
    if (isDuplicate) {
      return res.status(409).json({ success: false, message: 'An intern with this name already exists' });
    }
    ...
    if (!internId) {
      ...
      for (let i = 0; i < length - 1; i++) {
        internId += characters.charAt(Math.floor(Math.random() * characters.length));
      }
    }
    
    await Intern.createIntern(internId, firstName, middleInitial, lastName, suffix, schoolId, courseId);
    res.status(201).json({ success: true, message: 'Intern created successfully', data: { id: internId } });
  } catch (err) {
    console.error(err);
...
exports.updateIntern = async (req, res) => {
  try {
    const { firstName, middleInitial, lastName, suffix, schoolId, courseId } = req.body;
    const id = req.params.id;
    
    // Check for duplicate intern with same first and last name (excluding current record)
    const isDuplicate = await Intern.checkDuplicateByNameExcludingId(firstName, lastName, id);
    if (isDuplicate) {
      return res.status(409).json({ success: false, message: 'An intern with this name already exists' });
    }
    
    const affectedRows = await Intern.updateIntern(id, firstName, middleInitial, lastName, suffix, schoolId, courseId);
    
    if (affectedRows === 0) {
      return res.status(404).json({ success: false, message: 'Intern not found' });
    }
    
    res.status(200).json({ success: true, message: 'Intern updated successfully' });
  } catch (err) {
    console.error(err);

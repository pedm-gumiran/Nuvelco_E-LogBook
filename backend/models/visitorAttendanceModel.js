...
exports.getAllVisitorAttendance = async () => {
  const [rows] = await db.execute(
    "SELECT id, visitor_name, time_in, date, purpose, address, company_name FROM visitor_attendance ORDER BY date DESC, time_in DESC",
  );
  return rows;
};
...
exports.recordAttendance = async (visitorId) => {
  try {
    ...
    console.log("Visitor attendance check:", {
      visitorName,
      today,
...
/* CREATE */
exports.createVisitorAttendance = async (
  visitorName,
  timeIn,
  timeOut,
  purpose,
  ...
  const [result] = await db.execute(
    "INSERT INTO visitor_attendance (visitor_name, time_in, purpose, address, company_name) VALUES (?, ?, ?, ?, ?)",
    [
      visitorName,
      timeIn,
      purpose || null,
...
exports.deleteVisitorAttendance = async (id) => {
  const [result] = await db.execute(
    "DELETE FROM visitor_attendance WHERE id = ?",
    [id],
  );
  return result.affectedRows;
};

  ...
  try {
    ...
    if (existingRows.length === 0) {
      ...
      const [result] = await db.execute(
        'INSERT INTO intern_attendance (intern_name, course_name, school_name, am_in, am_in_image, am_out, pm_in, pm_out) VALUES (?, ?, ?, ?, ?, NULL, NULL, NULL)',
        [internName, courseName, schoolName, currentTime, photo]
      );
...
/* CREATE */
exports.createInternAttendance = async (internName, amIn, amOut, pmIn, pmOut) => {
  const [result] = await db.execute(
...
exports.deleteInternAttendance = async (id) => {
  const [result] = await db.execute('DELETE FROM intern_attendance WHERE id = ?', [id]);
  return result.affectedRows;
};

/* DELETE BY COURSE */
exports.deleteInternAttendanceByCourse = async (courseName) => {
  const [result] = await db.execute('DELETE FROM intern_attendance WHERE course_name = ?', [courseName]);
  return result.affectedRows;
};

/* DELETE BY INTERN NAME */
exports.deleteInternAttendanceByInternName = async (internName) => {
  const [result] = await db.execute('DELETE FROM intern_attendance WHERE intern_name = ?', [internName]);
  return result.affectedRows;
};

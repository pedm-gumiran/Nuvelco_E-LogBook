...
import { FiPlus, FiDownload, FiRotateCcw, FiRefreshCw, FiLoader } from 'react-icons/fi'
...
export default function School() {
  ...
  const schoolFields = [
    { name: 'school_name', label: 'School Name', type: 'text', required: true, placeholder: 'Enter school name' }
  ]
  ...
  const handleAddSubmit = async (formData) => {
    try {
      ...
      if (err.response?.status === 409) {
        toast.error(err.response.data.message || 'A school with this name already exists')
      } else {
  ...
  const handleEditSubmit = async (formData) => {
    try {
      if (selectedSchool) {
        ...
        const response = await axios.put(`/school/${selectedSchool.id}`, updatedSchool)
        if (response.data.success) {
          // Update local state with new school name and current timestamp
          setSchoolData(prev => prev.map(s => s.id === selectedSchool.id ? { 
            ...s, 
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-yellow-50">
      ...
      <EditModal
        ...
        onClose={() => { setIsEditModalOpen(false); setSelectedSchool(null) }}
      ...
      <ViewModal
        ...
        onClose={() => { setIsViewModalOpen(false); setSelectedSchool(null) }}
        data={selectedSchool}
        itemName="School"
        fields={schoolFields}
        size="md"

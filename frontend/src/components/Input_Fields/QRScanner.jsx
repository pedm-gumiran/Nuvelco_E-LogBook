...
import { FiCamera, FiCameraOff, FiSearch } from "react-icons/fi";
      ...
      const getDevices = async () => {
        try {
          ...
          if (devices.length > 0) {
            ...
            setSelectedCamera(
              physicalCamera ? physicalCamera.id : devices[0].id,
            );
          }
        } catch (err) {
    ...
    <div className={`relative ${className}`}>
                  ...
                  {isInsecureContext ? (
                    <>
                      ...
                      <p className="text-xs text-white/80 mb-4">
                        Camera access is blocked because this page is not using
                        HTTPS.
                      </p>
                      <button
      ...
      {isOpen && isScanning && (
        <div className="mt-4 text-center">
          ...
          {/* Manual Search Link - Below Scan Now */}
          {onManualSearch && (
            <button
              onClick={onManualSearch}
              className="-mt-1 flex items-center gap-2 mx-auto text-white/80 hover:text-white font-medium underline underline-offset-4 decoration-transparent hover:decoration-white transition-all text-sm"
            >
              <FiSearch size={14} />

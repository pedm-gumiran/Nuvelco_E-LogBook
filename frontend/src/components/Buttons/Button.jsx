...
export default function Button({
  ...
  const getVariantClasses = () => {
    ...
    switch (variant) {
      ...
      case "modal-primary":
        return "bg-[#188b3e] text-white hover:bg-[#147a35]";
  ...
  // Auto-add FiSave icon for modal-primary variant if no icon provided (icon !== null to allow explicit disabling)
  const buttonIcon =
    icon !== undefined ? (
      icon
  ...
  return (
    ...
    >
      ...
      {!isLoading && buttonIcon && (
        <span className="flex items-center">{buttonIcon}</span>
      )}

      {/* Change text depending on loading state */}
      <span className={hideLabelOnSmall ? "hidden sm:inline" : ""}>

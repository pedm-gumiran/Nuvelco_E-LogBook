...
import { FiUsers, FiPlus, FiFilter, FiDownload, FiTrash2, FiEdit2, FiRotateCcw, FiEye, FiRefreshCw, FiX, FiLoader } from 'react-icons/fi'
...
export default function InternModal({ isOpen, onClose, school, course, onInternAdded }) {
  ...
  const internFields = [
    { name: 'intern_id', label: 'Intern ID', type: 'text', required: false, placeholder: 'Auto-generated', disabled: true },
    ...
    { name: 'school_id', label: 'School ID', type: 'text', required: false, disabled: true, hidden: true },
    { name: 'course_id', label: 'Course ID', type: 'text', required: false, disabled: true, hidden: true }
  ]

  // Field configuration for EditModal - course is auto-set since editing intern inside course
  const internEditFields = [
    { name: 'intern_id', label: 'Intern ID', type: 'text', required: false, disabled: true },
    { name: 'firstname', label: 'First Name', type: 'text', required: true, placeholder: 'Enter first name' },
    { name: 'middle_initial', label: 'Middle Initial', type: 'text', required: false, placeholder: 'Enter middle initial' },
    { name: 'lastname', label: 'Last Name', type: 'text', required: true, placeholder: 'Enter last name' },
    { name: 'suffix', label: 'Suffix', type: 'text', required: false, placeholder: 'Enter suffix (e.g., Jr., Sr., III)' },
  ...
  const filteredData = internData.filter(intern => {
    const fullName = `${intern.firstname} ${intern.middle_initial ? intern.middle_initial + '. ' : ''}${intern.lastname} ${intern.suffix}`.trim()
    const matchesSearch = intern.intern_id.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleAddSubmit = async (formData) => {
    try {
      ...
      let newInternId = 'I'
      for (let i = 0; i < length - 1; i++) {
        newInternId += characters.charAt(Math.floor(Math.random() * characters.length))
      }
  ...
  const handleEditSubmit = async (formData) => {
    try {
      if (selectedIntern) {
        ...
        const response = await axios.put(`/intern/${selectedIntern.id}`, updatedIntern)
        if (response.data.success) {
          setInternData(prev => prev.map(f => f.id === selectedIntern.id ? { ...formData, id: selectedIntern.id, intern_id: selectedIntern.intern_id } : f))
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/15">
      <div className="bg-white w-full h-full max-w-7xl mx-auto overflow-y-auto backdrop-blur-sm">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between z-10">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {course && school ? `${course.abbreviation || course.course_name} - ${school.school_name}` : 'Interns'}
            </h1>
        ...
        <EditModal
          ...
          onClose={() => { setIsEditModalOpen(false); setSelectedIntern(null) }}
        ...
        <ViewModal
          ...
          onClose={() => { setIsViewModalOpen(false); setSelectedIntern(null) }}
          data={selectedIntern}
          itemName="Intern"
          fields={internFields}
          size="md"
        ...
        <DeleteConfirmationModal
          ...
          onClose={() => { setIsDeleteModalOpen(false); setInternToDelete(null) }}
        ...
        <SummaryModal
          ...
          onClose={() => { setIsSummaryModalOpen(false); setNewlyAddedIntern(null) }}

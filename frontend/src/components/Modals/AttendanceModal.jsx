...
import { FiCalendar, FiClock, FiUsers, FiTrendingUp, FiUserPlus, FiAlertCircle } from 'react-icons/fi'
...
const AttendanceModal = ({ isOpen, onClose }) => {
  ...
  const fetchAttendanceData = async () => {
    ...
    try {
      ...
      const internData = internResponse.data.success ? internResponse.data.data : []
      const visitorData = visitorResponse.data.success ? visitorResponse.data.data : []
  ...
  const handleQRScan = async (data) => {
    ...
    try {
      ...
      if (personFound.type !== 'not_found') {
        personName = personFound.data.first_name + ' ' + personFound.data.last_name
      }
      ...
      if (personFound.type === 'intern') {
        ...
        if (checkResult && !checkResult.canRecord) {
          throw new Error(checkResult.message || 'Attendance already completed')
        }
      ...
      if (errorMsg.includes('already has complete attendance') || errorMsg.includes('Attendance already completed')) {
  ...
  const checkInternAttendanceStatus = async (internId) => {
    try {
      ...
      const attendanceRes = await axiosInstance.get('/intern-attendance/today/all')
      
      if (attendanceRes.data.success) {
        const todayRecords = attendanceRes.data.data.filter(
          record => record.intern_name === internName && 
  ...
  const handleCameraSave = async (capturedImage) => {
    ...
    try {
      ...
      const result = await handleInternAttendance(pendingIntern.id, capturedImage)
    ...
    cancelPreviousSpeech()
    speechTimeoutRef.current = setTimeout(() => {
      speak(isValid ? 'Attendance Saved. Thank You very much' : 'Attendance Unsuccessful', {
        rate: 0.8,
        pitch: 1,
        volume: 5
      })
  ...
  const getAvatarColor = (index) => {
    const colors = ['bg-[#188b3e]', 'bg-purple-500', 'bg-orange-500', 'bg-green-500', 'bg-red-500', 'bg-indigo-500'];
  ...
  return (
    <div className="fixed inset-0 bg-gray-50 z-50 flex flex-col overflow-hidden">
      ...
      <div className="flex-none flex items-center justify-between p-4 md:p-3 border-b border-gray-200 bg-white shadow-sm z-10">
        <div className="flex items-center gap-4 text-sm md:text-xl font-bold tracking-tight text-gray-700">
          <div className="flex items-center gap-2">
            ...
            <span className="uppercase tracking-widest text-[10px] md:text-sm">{formatDate(currentTime)}</span>
          </div>
        </div>
        <div className="flex items-center gap-4 md:gap-6">
          <div className="flex items-center gap-2 font-bold text-gray-700">
            <FiClock className="text-[#188b3e]" />
            <span className="tabular-nums uppercase tracking-widest text-[10px] md:text-sm">{formatTime(currentTime)}</span>
          </div>
          <Btn_X onClick={onClose} className="p-2 hover:bg-gray-100 rounded-lg transition-colors" />
        </div>
      </div>
          ...
          <div className="lg:w-5/12 w-full flex flex-col items-center">
            {activeTab === 'visitors' ? (
              ...
              <div className="w-full max-w-md bg-white rounded-2xl p-6 border border-gray-200 shadow-xl">
                <h3 className="text-lg font-black text-gray-800 uppercase tracking-widest mb-4">Visitor Attendance</h3>
                  ...
                  <div className="aspect-square w-full rounded-2xl overflow-hidden bg-[#1a2b4b] shadow-xl flex items-center justify-center p-2 sm:p-4 border border-gray-300">
                    <div className="relative w-full h-full rounded-xl overflow-hidden">
                      <QRScanner 
                        ...
                        isOpen={isOpen} 
                        disabled={showRegisterUser || showSummary || showCameraModal || isCameraSaving || (scannedData && typeof scannedData === 'object')}
                      />
          ...
          <div className="lg:w-7/12 w-full space-y-4">
            ...
            <div className="bg-white rounded-xl p-4 sm:p-6 border-2 border-gray-200 shadow-xl">
              <h2 className="text-xs uppercase tracking-[0.2em] text-gray-600 font-black mb-3">
                {activeTab === 'faculty' ? 'TOTAL INTERNS TODAY' : 'TOTAL VISITORS TODAY'}
              </h2>
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-baseline gap-2">
                    <span className="text-3xl sm:text-4xl font-black tracking-tighter" style={{ color: activeTab === 'faculty' ? '#188b3e' : '#ea580c' }}>
                      {activeTab === 'faculty' ? attendanceData.todayStats.faculty : attendanceData.todayStats.visitors}
                    </span>
                    <span className="text-gray-500 font-bold text-xs uppercase tracking-widest">Active Records</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                <div className="flex justify-between items-start mb-2">
                  <h2 className="text-[10px] uppercase tracking-widest text-gray-500 font-black">THIS WEEK</h2>
                  <span className="text-[8px] px-1.5 py-0.5 rounded font-bold uppercase tracking-tighter" style={{ backgroundColor: activeTab === 'faculty' ? '#dcfce7' : '#ffedd5', color: activeTab === 'faculty' ? '#188b3e' : '#ea580c' }}>
                    {activeTab === 'faculty' ? 'Intern' : 'Visitor'}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <FiUsers style={{ color: activeTab === 'faculty' ? '#188b3e' : '#ea580c' }} />
                  <span className="text-xl font-black text-gray-800 tracking-tight">
                    {activeTab === 'faculty' ? attendanceData.todayStats.faculty : attendanceData.weeklyStats.visitors}
                  </span>
                </div>
              </div>
              <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                <div className="flex justify-between items-start mb-2">
                  <h2 className="text-[10px] uppercase tracking-widest text-gray-500 font-black">THIS MONTH</h2>
                  <span className="text-[8px] px-1.5 py-0.5 rounded font-bold uppercase tracking-tighter" style={{ backgroundColor: activeTab === 'faculty' ? '#dcfce7' : '#ffedd5', color: activeTab === 'faculty' ? '#188b3e' : '#ea580c' }}>
                    {activeTab === 'faculty' ? 'Intern' : 'Visitor'}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                   <FiUsers style={{ color: activeTab === 'faculty' ? '#188b3e' : '#ea580c' }} />
                  <span className="text-xl font-black text-gray-800 tracking-tight">
                    {activeTab === 'faculty' ? attendanceData.todayStats.faculty : attendanceData.monthlyStats.visitors}
                  </span>
                </div>
              </div>
              <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                <div className="flex justify-between items-start mb-2">
                  <h2 className="text-[10px] uppercase tracking-widest text-gray-500 font-black">YEARLY TOTAL</h2>
                  <span className="text-[8px] px-1.5 py-0.5 rounded font-bold uppercase tracking-tighter" style={{ backgroundColor: activeTab === 'faculty' ? '#dcfce7' : '#ffedd5', color: activeTab === 'faculty' ? '#188b3e' : '#ea580c' }}>
                    {activeTab === 'faculty' ? 'Intern' : 'Visitor'}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <FiUsers style={{ color: activeTab === 'faculty' ? '#188b3e' : '#ea580c' }} />
                  <span className="text-xl font-black text-gray-800 tracking-tight">
                    {activeTab === 'faculty' ? attendanceData.todayStats.faculty : attendanceData.yearlyStats.visitors}
                  </span>
                </div>
              </div>
            </div>
                ...
                {activeTab === 'faculty' ? (
                  <>
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="text-xs uppercase tracking-widest text-gray-600 font-black">Recent Attendance - Interns</h2>
                      <span className="text-[10px] text-gray-500 font-medium">{formatDate(currentTime)}</span>
                    </div>
                    ...
                    <div className="space-y-2 max-h-[280px] overflow-y-auto pr-1">
                      {attendanceData.facultyToday.length > 0 ? (
                        attendanceData.facultyToday.map((intern, index) => (
                          <div key={intern.id} className="grid grid-cols-[1fr_60px_60px_60px_60px] gap-2 items-center p-2 hover:bg-gray-50 rounded-lg transition-colors">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <p className="text-sm font-bold text-gray-800 leading-tight">{intern.intern_name}</p>
                                <p className="text-[9px] text-gray-500">Intern</p>
                              </div>
                            </div>
                            <div className="text-center">
                              <div className="text-xs text-gray-700">{to12HourFormat(intern.am_in)}</div>
                            </div>
                            <div className="text-center">
                              <div className="text-xs text-gray-700">{to12HourFormat(intern.am_out)}</div>
                            </div>
                            <div className="text-center">
                              <div className="text-xs text-gray-700">{to12HourFormat(intern.pm_in)}</div>
                            </div>
                            <div className="text-center">
                              <div className="text-xs text-gray-700">{to12HourFormat(intern.pm_out)}</div>
                            </div>
                          </div>
                        ))
                      ) : (
                  ...
                  <>
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="text-xs uppercase tracking-widest text-gray-600 font-black">Recent Attendance - Visitors</h2>
                      <span className="text-[10px] text-gray-500 font-medium">{formatDate(currentTime)}</span>
                    </div>
                    ...
                    <div className="space-y-2 max-h-[280px] overflow-y-auto pr-1">
                      {attendanceData.visitorsToday.length > 0 ? (
                        attendanceData.visitorsToday.map((visitor, index) => (
                          <div key={visitor.id} className="grid grid-cols-[1fr_80px_80px] gap-2 items-center p-2 hover:bg-gray-50 rounded-lg transition-colors">
                            <div className="flex items-center gap-2">
                              <div className="relative">
                                <div className={`w-8 h-8 rounded-full ${getAvatarColor(index)} flex items-center justify-center text-white font-bold text-xs`}>
                              ...
                              <div>
                                <p className="text-sm font-bold text-gray-800 leading-tight">{visitor.visitor_name}</p>
                                <p className="text-[9px] text-gray-500">Visitor</p>
                              </div>
                            </div>
                            <div className="text-center text-xs text-gray-700">{formatTimeWithPeriod(visitor.time_in)}</div>
                            <div className="text-center text-xs text-gray-700 truncate">{visitor.purpose || '--'}</div>
                          </div>
                        ))
                      ) : (
                        <div className="text-center py-4 text-gray-400 text-sm">
      ...
      {scannedData && typeof scannedData === 'object' && (
        <div className={`fixed bottom-0 left-0 right-0 p-4 md:p-8 border-t shadow-2xl z-50 animate-in fade-in slide-in-from-bottom duration-500 ${
          scannedData.isValid ? 
            (scannedData.personType === 'faculty' ? 'bg-green-600 border-green-400' : 'bg-green-600 border-green-400') : 
          ...
          <div className="max-w-md mx-auto text-center">
            ...
            <h3 className="text-xl md:text-3xl font-black text-white mb-1 md:mb-2 tracking-tight uppercase">
              {scannedData.isValid ? 'Welcome!' : scannedData.personType === 'completed' ? 'Warning' : 'Please Register'}
            </h3>
            <div className="bg-white/10 backdrop-blur-md rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/20">
              <p className="text-white text-lg md:text-2xl font-bold tracking-wide break-words">
                {scannedData.isValid ? scannedData.personName || scannedData.originalData : scannedData.personName || scannedData.originalData}
              </p>
              <div className="mt-2 md:mt-4 flex items-center justify-center gap-2 text-white/80 font-medium uppercase tracking-widest text-[10px] md:text-xs">
                {scannedData.isValid ? (
                  <>
                    ...
                    <span>
                      {scannedData.personType === 'faculty' ? 'Intern Attendance Recorded' : 'Visitor Attendance Recorded'}
                    </span>
                  </>
                ) : scannedData.personType === 'not_found' || scannedData.personType === 'error' ? (
      ...
      <SummaryModal
        isOpen={showSummary}
        onClose={() => { setShowSummary(false); setNewlyRegisteredVisitor(null) }}

...
const DeleteConfirmationModal = ({
  ...
  const effectiveLoadingText = loadingText || (type === 'delete' ? 'Deleting...' : 'Resetting...')
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md mx-4 bg-white rounded-3xl shadow-2xl animate-fadeIn flex flex-col max-h-[85vh]">
        {/* Header */}
        <div className={`flex items-center justify-between px-6 py-4 ${headerGradient} shrink-0 rounded-t-3xl`}>
        ...
        <div className="p-6 flex-1 min-h-0 overflow-y-auto">
          <div className="flex items-start gap-3">
            ...
            <p className="text-gray-600 text-sm leading-relaxed">
              {message || `Are you sure you want to ${type} this ${itemName.toLowerCase()}? This action cannot be undone.`}
            </p>
          </div>
        </div>
        ...
        <div className="px-6 py-4 bg-gray-50 shrink-0 rounded-b-3xl">
          <div className="flex items-center justify-end gap-3">
            ...
            <Button
              ...
              customColor="#dc2626"
              onClick={onConfirm}
              label={isLoading ? effectiveLoadingText : (type === 'delete' ? 'Delete' : 'Reset')}
              icon={isLoading ? null : (type === 'delete' ? <FiTrash2 className="w-4 h-4" /> : <FiRotateCcw className="w-4 h-4" />)}
              isLoading={isLoading}

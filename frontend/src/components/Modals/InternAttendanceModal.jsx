...
const InternAttendanceModal = ({
  ...
  const filteredInterns = Object.values(groupedInterns).filter((intern) => {
    const matchesName = intern.intern_name
      ?.toLowerCase()
  ...
  const handleConfirmReset = async () => {
    ...
    try {
      ...
      if (course?.course_name) {
        await axios.delete(
          `/intern-attendance/course/${encodeURIComponent(course.course_name)}`,
        );
        toast.success(
          `Attendance records for ${course.course_name} have been reset`,
        );
      } else {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await axios.delete(
        `/intern-attendance/intern/${encodeURIComponent(selectedInternName)}`,
      );
      toast.success(
        `Attendance records for ${selectedInternName} have been deleted`,
      );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/15">
      <div className="bg-white w-full h-full max-w-7xl mx-auto overflow-y-auto backdrop-blur-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {filteredInterns.map((intern) => (
                <ItemCard
                  ...
                  onMenuToggle={() =>
                    setOpenMenuId((prev) =>
                      prev === intern.intern_name ? null : intern.intern_name,
                    )
                  }
        ...
        <DeleteConfirmationModal
          ...
          message={
            selectedInternName
              ? `Are you sure you want to delete attendance records for ${selectedInternName}? This action cannot be undone.`
